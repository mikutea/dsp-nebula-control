...
- Deployment and DataRoot recovery refuse to proceed or report ready while the
  retired privileged switching task from an older release remains registered.
  Remove it with that previously installed release before upgrading.
...
| --- | --- |
| Viewer | Read status, jobs, telemetry, player presence, redacted console, saves, configuration, and update/mod inventory. |
