...
describe('production configuration', () => {
  ...
  it('accepts only a bounded deployment release identifier', () => {
    expect(loadConfig({
      NODE_ENV: 'test', DYSON_DEPLOYMENT_VERSION: 'v0.1.0-rc.24+fixture'
    }).deploymentVersion).toBe('v0.1.0-rc.24+fixture')
    expect(() => loadConfig({
  ...
  it('defaults the expected Bridge heartbeat to the full repository release version', () => {
    expect(loadConfig({ NODE_ENV: 'test' }).bridgePluginVersion).toBe('0.1.0-rc.24')
  })
