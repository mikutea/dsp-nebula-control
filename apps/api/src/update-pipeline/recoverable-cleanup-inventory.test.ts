import { mkdtemp, mkdir, writeFile, rename, rm, link } from 'node:fs/promises'
...
it('keeps a release tree identity across quarantine and detects renamed content', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'cleanup-tree-'))
  try {
...
it('rejects hardlinked material instead of trusting shared bytes', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'cleanup-link-'))
  try {
