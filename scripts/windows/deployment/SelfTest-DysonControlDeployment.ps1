...
function Test-DysonBrokerProfileFileAclRestore {
    ...
    foreach($name in @('Assert-DysonDeploymentPlainFile','Set-DysonLifecycleBrokerDeploymentFileBytesAtomic',
        'Restore-DysonLifecycleBrokerDeploymentPreimage')) {
    ...
    foreach($name in @('Assert-DysonUninstallPlainFile','Set-DysonUninstallLifecycleFileBytesAtomic',
        'Restore-DysonUninstallLifecyclePreimageAclsAndTask')) {
...
try {
    ...
    $global:DysonDeploymentTaskFixture[$retiredTaskName] = [pscustomobject]@{
        FixtureTaskName = $retiredTaskName; TaskName = $retiredTaskName; TaskPath = '\'; State = 'Ready'; Xml = '<RetiredTask />'
    }
    $retiredTaskRejected = $false
    try { [void](Assert-DysonRetiredPrivilegedRuntimeAbsent) } catch { $retiredTaskRejected = $true }
    Assert-SelfTest -Condition $retiredTaskRejected `
        -Message 'the deployment preflight accepted a retired privileged control task'
    [void]$global:DysonDeploymentTaskFixture.Remove($retiredTaskName)
    foreach ($protectedScript in @($installScript,$deploymentScript,$uninstallScript,$statusScript)) {
        Assert-SelfTest -Condition ([IO.File]::ReadAllText($protectedScript).Contains('Assert-DysonRetiredPrivilegedRuntimeAbsent')) `
            -Message ('a deployment entry point omitted the retired privileged task gate: ' + [IO.Path]::GetFileName($protectedScript))
    }
    ...
    Assert-SelfTest -Condition ($installPreview.state -eq 'preview') `
        -Message 'installer WhatIf did not return a preview receipt'
    ...
    Assert-SelfTest -Condition ($installerResult.state -eq 'installed') `
        -Message 'the reusable installer did not complete'
