...
try {
    ...
    if ($RegisterStartupTask) {
        & (Join-Path $PSScriptRoot 'Install-DysonControlTask.ps1') `
            -InstallRoot $installFull `
            -DataRoot $dataFull `
    ...
    try {
        ...
        throw ('Installation failed ({0}); automatic rollback also failed ({1}).' -f $installError.Exception.Message, $_.Exception.Message)
    }
    throw $installError
}
