...
export const baseline = 'ee81cc3e70737440bca2d6b8b0f3550f4ebe8877'
...
export const componentBaselines = Object.fromEntries(
  ['status', 'recovery', 'bootstrap', 'lifecycle', 'deployment'].map(group => [group, {
    commit: baseline,
    evidence: 'https://github.com/mikutea/dsp-nebula-control/actions/runs/34891633662'
  }])
)
...
export function classifyChange(file, before, after) {
  ...
  if (file === 'scripts/public-release/policy.mjs' &&
      aclSourceHash(after) === '30fddda7fa9993bd8a9e3986c3166379693031a32759466f245a1568b0c9d82e') return 'reviewed-hygiene-policy'
...
export function selectCommands(changes, { componentChanges } = {}) {
  ...
  if (changes.some(change => change.kind === 'affected' && change.file.startsWith('scripts/windows/bridge/'))) {
    commands.push(['powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass',
...
const apiBuildCommand = [process.execPath,
  [npmCliPath, '--prefix', 'apps/api', 'run', 'build']]
